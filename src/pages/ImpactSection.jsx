        ...
        <div className="max-w-3xl mx-auto"> {/* Max width for the content block */}
          ...
          <div className="flex flex-wrap justify-center gap-3 sm:gap-4">
            {clientNames.map((name, index) => (
              <div
                key={index}
                className="bg-surface/70 backdrop-blur-sm text-text-main text-xs sm:text-sm font-medium px-3 py-1.5 rounded-md shadow-md border border-white/10 hover:bg-surface/90 transition-colors cursor-default"
              >
                {name}
              </div>
            ))}
